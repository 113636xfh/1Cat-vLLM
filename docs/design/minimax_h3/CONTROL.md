...
PR #564 synchronizes with `onecat/main@e7fa44deb253`, which already contains
native H3 (#557), SM70 operators (#558) and workflows/adapters (#565).
Only this control document needs manual conflict resolution; both backend
histories and the workflow records are retained. Native FlashInfer/W8 CUDA
sources and binaries are unchanged from the measured local-rotation build.

The integrated video suite passes **266 tests, 2 skipped, 1 deselected**, using
the preceding FlashInfer suite command. The INT8 local-rotation and default
unquantized TP4 block modules each pass on every rank in separate torchrun
processes. Raw commands and logs are retained in the campaign's
`feeding-gather/merge-main/`. The 61.538397-second result remains the preceding
