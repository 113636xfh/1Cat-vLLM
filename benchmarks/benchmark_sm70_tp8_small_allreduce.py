...
def _check_output(output: torch.Tensor) -> float:
    expected = dist.get_world_size() * (dist.get_world_size() + 1) / 2
    return float((output.float() - expected).abs().max().item())
...
def main() -> int:
    ...
    for elements in args.elements:
        ...
        custom_graph.replay()
        torch.cuda.synchronize()
        nccl_error = _check_output(nccl_out)
        custom_error = _check_output(custom_out)
        ...
        for _ in range(args.repeats):
            ...
            custom_local = _measure_graph(custom_graph, args.warmup, args.iterations)
        ...
        results.append(
            {
                ...
                "nccl_max_abs": nccl_error,
                "custom_max_abs": custom_error,
            }
        )
