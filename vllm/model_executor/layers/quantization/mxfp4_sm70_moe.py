...
def _select_mxfp4_direct_order_offsets(
    ...
    """Return the immutable one-row offsets for B1 direct-order decode.

    Active-expert compaction overwrites ``compact_expert_offsets`` during
    M2--M8 warmup and verifier calls. Direct-order B1 has exactly one row per
    route, so it must use the separately maintained slot offsets instead.
    """
