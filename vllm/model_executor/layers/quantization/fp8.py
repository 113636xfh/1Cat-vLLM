...
_SM70_DSV4_FP8_QPN8_CONFIGS = {
    # Real PP2 x TP4 B1 winners: (split-K, accumulator chains, prefetch).
...
_SM70_DSV4_FP8_QPN8_SHAPES = {
    # suffix: accepted (layer TP size, K, N) tuples. The replicated indexer
    # wq_b is deliberately excluded: long-prefill runs it concurrently with
    # the main wq_b, so one shared dense fallback workspace would be unsafe.
...
_SM70_DSV4_FP8_QPN8_WORKSPACE_ELEMENTS = max(
    k * n for k, n, _ in _SM70_DSV4_FP8_QPN8_CONFIGS
)
...
_sm70_dsv4_fp8_qpn8_workspaces: dict[tuple[int, torch.dtype], torch.Tensor] = {}
...
def _is_sm70_dsv4_fp8_qpn8_runtime_contract() -> bool:
    """Require the measured serialized PP2 x TP4 no-spec route."""
    vllm_config = get_current_vllm_config()
...
def _sm70_dsv4_fp8_qpn8_config(
    layer: torch.nn.Module, *, gated_silu: bool
) -> tuple[int, int, bool] | None:
    if getattr(layer, "weight_block_size", None) != [128, 128]:
        return None
    suffix = getattr(layer, "prefix", "").rsplit(".", 1)[-1]
    accepted = _SM70_DSV4_FP8_QPN8_SHAPES.get(suffix)
    if accepted is None:
        return None
    ...
    if gated_silu:
        ...
        if (
            ...
            return None
    return _SM70_DSV4_FP8_QPN8_CONFIGS.get((k_dim, n_dim, gated_silu))


def _sm70_dsv4_fp8_qpn8_bmm_config(
    ...
    if (
        ...
        return None
    return _SM70_DSV4_FP8_QPN8_CONFIGS[(4096, 1024, False)]
...
def _get_sm70_dsv4_fp8_qpn8_workspace(
    weight: torch.Tensor,
) -> torch.Tensor | None:
    """Allocate one memory-bounded FP16 prefill fallback per device."""
    device_index = weight.device.index
    if device_index is None:
        device_index = torch.accelerator.current_device_index()
    cache_key = (device_index, torch.float16)
    workspace = _sm70_dsv4_fp8_qpn8_workspaces.get(cache_key)
    if workspace is not None:
        return workspace
    try:
        workspace = torch.empty(
            (_SM70_DSV4_FP8_QPN8_WORKSPACE_ELEMENTS,),
            dtype=torch.float16,
            device=weight.device,
        )
    except torch.OutOfMemoryError:
        logger.warning_once(
            "Insufficient memory for the bounded DeepSeek V4 QPN8 prefill "
            "workspace; retaining TurboMind FP8."
        )
        return None
    _sm70_dsv4_fp8_qpn8_workspaces[cache_key] = workspace
    return workspace
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        if self.use_sm70_fp8_turbomind:
            ...
            if getattr(layer, "is_bmm", False):
                dsv4_qpn8_bmm_config = (
                    _sm70_dsv4_fp8_qpn8_bmm_config(layer)
                    if envs.VLLM_SM70_DSV4_FP8_QPN8
                    else None
                )
                dsv4_qpn8_bmm_runtime = (
                    _is_sm70_dsv4_fp8_qpn8_runtime_contract()
                    if dsv4_qpn8_bmm_config is not None
                    else False
                )
                ...
                if dsv4_qpn8_bmm_config is not None and not dsv4_qpn8_bmm_runtime:
                    logger.info_once(
                        "DeepSeek V4 grouped QPN8 retains TurboMind outside the "
                        "PP2 x TP4 single-request no-spec runtime contract."
                    )
                if dsv4_qpn8_bmm_config is not None and dsv4_qpn8_bmm_runtime:
                    missing_ops = _missing_sm70_fp8_qpn8_ops()
                    if missing_ops:
                        raise RuntimeError(
                            "VLLM_SM70_DSV4_FP8_QPN8=1 requires the "
                            "source-built SM70 QPN8 extension; missing ops: "
                            f"{missing_ops}."
                        )
                    workspace = _get_sm70_dsv4_fp8_qpn8_workspace(weight)
                    if workspace is not None:
                        qpn8_weights = []
                        qpn8_scales = []
                        ...
                        split_k, nacc, prefetch = dsv4_qpn8_bmm_config
                        ...
                        logger.info_once(
                            "Memory-neutral SM70 DeepSeek V4 grouped QPN8 path "
                            "enabled for TP4 wo_a."
                        )
                        return
            ...
            dsv4_qpn8_config = (
                _sm70_dsv4_fp8_qpn8_config(layer, gated_silu=False)
                if envs.VLLM_SM70_DSV4_FP8_QPN8
                else None
            )
            dsv4_qpn8_candidate = dsv4_qpn8_config is not None
            qpn8_candidate_layer = generic_qpn8_candidate or dsv4_qpn8_candidate
            qpn8_runtime = bool(
                (dsv4_qpn8_candidate and _is_sm70_dsv4_fp8_qpn8_runtime_contract())
            ...
            if qpn8_candidate_layer and qpn8_runtime:
                dsv4_gated_config = None
                if dsv4_qpn8_candidate and use_gated_silu:
                    dsv4_gated_config = _sm70_dsv4_fp8_qpn8_config(
                        layer, gated_silu=True
                    )
                    if dsv4_gated_config is None:
                        raise RuntimeError(
                            "DeepSeek V4 QPN8 gate/up layer violated its "
                            "accepted fused-SiLU contract."
                        )
                missing_ops = _missing_sm70_fp8_qpn8_ops()
                if missing_ops:
                    explicit_flag = (
                        "VLLM_SM70_DSV4_FP8_QPN8"
                        if dsv4_qpn8_candidate
                        else "VLLM_SM70_FP8_QPN8"
                    )
                    if os.getenv(explicit_flag) is not None:
                        raise RuntimeError(
                            f"{explicit_flag}=1 requires the source-built SM70 "
                ...
                if not missing_ops and workspace is not None:
                    ...
                    if dsv4_qpn8_candidate:
                        assert dsv4_qpn8_config is not None
                        split_k, nacc, prefetch = dsv4_qpn8_config
                    else:
                    ...
                    if use_gated_silu:
                        if dsv4_qpn8_candidate:
                            assert dsv4_gated_config is not None
                    ...
                    if dsv4_qpn8_candidate:
                        logger.info_once(
                            "Memory-neutral SM70 DeepSeek V4 QPN8 path enabled "
                            "for accepted PP2 x TP4 no-spec operator shapes."
                        )
                    else:
                        logger.info_once(
